...
function AddNewResGroupForm(props) {
  ...
  const GroupTypeValues = ["NAME", "QUESTION", "RESPONSE", "YEAR"];
  ...
  const OptionFields = ({ groupType }) => {
    ...
    switch (groupType) {
      case "NAME":
        fields = (
          <Form.Group as={Row} controlId="customSelectResponses">
            ...
            <Col sm="10">
              <Form.Control as="select" multiple ref={givenResponsesRef}>
                {responses.map((res, i) => (
                  <option
                    key={i}
                    value={res.id}
      ...
      case "YEAR":
        {
          const years = ["2012", "2015", "2020", "2022"];
  ...
  console.log(request.respondantAnswer);
  const modal = (
    <>
      <FormModal
        show={show}

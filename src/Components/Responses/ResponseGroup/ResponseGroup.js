...
import NewResponseGroupBuilder from "../../../Containers/ResponsesBuilder/UpdateResponseGroupBuilder/NewResponseGroupBuilder";
...
import DeleteResponseGroupBuilder from "../../../Containers/ResponsesBuilder/UpdateResponseGroupBuilder/DeleteResponseGroupBuilder";
...
function ResponseGroup(props) {
  ...
  const { survey = {}, responses = [], responseGroups = [] } = props;
  ...
  const [request, setRequest] = useState({
    activeResponse: null,
    showDeleteResponseGroupModal: false,
  ...
  const NonGroupResponse = ({ survey = {}, responses = [] }) => {
    let items = responses.reduce(
      (newItems, res) => {
        return res[`${exprt.props.IS_ARCHIVED}`] === false
          ? // &&  res[`${exprt.props.QUESTION_QRS_RESPONSE_GROUP_COUNT}`] === 0
            [...newItems, res]
          : newItems;
      },
                      ...
                      items.map((response, index) => (
                        <tr key={index}>
                          ...
                          <CustomOverlayTrigger
                            ...
                            title={
                              response[`${exprt.props.RESPONSE_DATE}`].split(
                                " "
                              )[1]
                            }
                          >
  ...
  const ResponseGroups = ({
    ...
    const getResGroupName = (resGroup) => {
      ...
      switch (resGroup.groupBy) {
        case "surveyId":
          groupedValue = survey[`${exprt.props.SURVEY_NAME}`];
          break;
        ...
        case "surveyId;questionId;rank;rankChoiceIndex":
          {
            ...
            survey[`${exprt.props.SECTION_LIST}`].forEach((section) => {
              section[`${exprt.props.QUESTION_LIST}`].forEach((question) => {
                if (question[`${exprt.props.QUESTION_ID}`] === questionId) {
                  valueInfo.sec = (
                    <Card.Text>
                      Section:{" "}
                      <strong>
                    ...
                    case "mulChoiceSelectionIndex":
                      valueInfo.answer = (
                        <Card.Text>
                          Answer:{" "}
                          {
                            question[`${exprt.props.MC_ANSWERS}`][
                              groupValues[2]
                            ]
                          }
                        </Card.Text>
                    ...
                    case "rank":
                      {
                        ...
                        valueInfo.answer = (
                          <Card.Text>
                            Choice:{" "}
                            {
                              question[`${exprt.props.RK_ANSWERS}`][
                                rankChoiceIndex
                              ]
                            }
                            <br />
                  ...
                  <span>
                    ...
                    >
                      <Button
                        variant="link"
                        size="lg"
                        className="p-0 m-0 mb-0 text-danger"
                      >
                        <FontAwesomeIcon
                          icon={["fas", "trash-alt"]}
                          size="sm"
